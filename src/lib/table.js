 ...
 */
export function rowLayout(text) {
    const pipes = [];
    ...
    if (from <= end && pipes[pipes.length - 1] !== end - 1)
        cells.push({ from, to: end });
    return { pipes, cells };
}
export function rowCells(text) {
    return rowLayout(text).cells;
}
/** A row's cell contents, trimmed of the padding the source carries. */
export function rowValues(text) {
...
export function rowAligns(text) {
...
export function createTable(columns, rows) {
    return formatTable({
        ...
        aligns: new Array(columns).fill('none'),
...
function isRowLine(text) {
    return text.trim().length > 0 && rowLayout(text).pipes.length > 0;
}
function sized(values, columns, blank) {
...
export function tableAt(lines, index) {
    if (index < 0 || index >= lines.length || !isRowLine(lines[index]))
        return null;
    ...
    if (!isRowLine(lines[start]) || isDelimiterRow(lines[start]))
        return null;
    let end = delimiter;
    ...
    const rows = [header];
    for (let n = delimiter + 1; n <= end; n += 1)
        rows.push(sized(rowValues(lines[n]), columns, ''));
    return {
        start,
        end,
...
export function cellIndexAt(text, offset) {
...
export function withColumn(table, at) {
...
export function withoutColumn(table, at) {
    if (table.aligns.length <= 1)
        return null;
    const aligns = table.aligns.slice();
    aligns.splice(at, 1);
    return {
