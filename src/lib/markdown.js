...
function precededByWord(tokens) {
    const previous = tokens?.[tokens.length - 1];
    if (!previous || typeof previous.raw !== 'string' || !previous.raw)
        return false;
...
const hashtag = {
    ...
    tokenizer(src, tokens) {
        ...
        if (!tag || !/[\p{L}\p{N}]/u.test(tag))
            return undefined;
        const raw = multi ? single[0] : `#${tag}`;
        const token = { type: 'hashtag', raw, tag };
        return token;
    },
    renderer(token) {
        return `<span class="md-hashtag">#${escapeHtml(token.tag)}</span>`;
    },
};
...
export function exportNoteHtml(title, text) {
    return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
