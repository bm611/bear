...
describe('loadLibrary', () => {
    ...
    it('adopts preferences left under the pre-rename key', () => {
        localStorage.setItem(LEGACY_KEY, JSON.stringify({ preferences: { ...defaultPreferences, theme: 'dark' }, filter: { kind: 'all' } }));
    ...
    it('ignores a stale sidebar flag when upgrading from the first schema', () => {
        ...
        localStorage.setItem(KEY, JSON.stringify({ version: 1, preferences: { ...defaultPreferences, sidebarVisible: true, font: 'serif' } }));
        const loaded = loadLibrary();
...
describe('backup files', () => {
    ...
    it('drops malformed notes and repairs missing fields', () => {
        const parsed = parseLibraryFile(JSON.stringify({ notes: [{ id: 'a', text: 'kept' }, { text: 'no id' }, null, 42] }));
