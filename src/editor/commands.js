...
export function toggleWrap(marker, endMarker = marker) {
    return (view) => {
        const { state } = view;
        const changes = [];
        const ranges = [];
        ...
        for (const range of state.selection.ranges) {
            ...
            if (wrapped) {
                changes.push({ from: range.from - marker.length, to: range.from, insert: '' }, { from: range.to, to: range.to + endMarker.length, insert: '' });
                ranges.push({
            ...
            changes.push({ from: range.from, insert: marker }, { from: range.to, insert: endMarker });
            ranges.push({
                anchor: range.from + offset + marker.length,
                head: range.to + offset + marker.length,
            });
        ...
        view.dispatch({
            changes,
            selection: EditorSelection.create(ranges.map((r) => EditorSelection.range(r.anchor, r.head)), state.selection.mainIndex),
            scrollIntoView: true,
...
function mapLines(view, map, test) {
    const { state } = view;
    ...
    const changes = lines
        ...
        let suffix = 0;
        while (suffix < line.text.length - prefix &&
    ...
    view.dispatch({
        changes: changeSet,
        selection: EditorSelection.create(state.selection.ranges.map((range) => EditorSelection.range(changeSet.mapPos(range.anchor, 1), changeSet.mapPos(range.head, 1))), state.selection.mainIndex),
        scrollIntoView: true,
        userEvent: 'input.format',
    });
    return true;
...
export const toggleTodo = (view) => mapLines(view, (text, allTodos) => {
    if (allTodos)
        return text.replace(TODO_PREFIX, '$1');
...
export const toggleBulletList = (view) => mapLines(view, (text, allBullets) => {
    if (allBullets)
        return text.replace(BULLET_PREFIX, '$1');
    const stripped = text.replace(TODO_PREFIX, '$1').replace(NUMBER_PREFIX, '$1');
    const indent = /^\s*/.exec(stripped)?.[0] ?? '';
    return `${indent}- ${stripped.slice(indent.length)}`;
}, BULLET_PREFIX);
export const toggleNumberedList = (view) => {
    let counter = 0;
    return mapLines(view, (text, allNumbered) => {
        if (allNumbered)
            return text.replace(NUMBER_PREFIX, '$1');
...
export const toggleQuote = (view) => mapLines(view, (text, allQuoted) => {
    if (allQuoted)
        return text.replace(QUOTE_PREFIX, '$1');
...
export function setHeading(level) {
    ...
    return (view) => mapLines(view, (text, allSame) => {
...
export const insertHorizontalRule = (view) => {
...
function isWrapped(state, marker, endMarker = marker) {
...
export function activeFormats(state) {
    const lines = selectedLines(state);
    const everyLine = (test) => lines.length > 0 && lines.every((line) => test.test(line.text));
    const levels = lines.map((line) => HEADING_PREFIX.exec(line.text)?.[2].length ?? 0);
    const heading = levels.length > 0 && levels[0] > 0 && levels.every((level) => level === levels[0])
        ? levels[0]
        : null;
