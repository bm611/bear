...
import { applyTableAction, nextTableCell, previousTableCell, type TableAction } from './tables'

/** Same stand-in as `commands.test.ts`: these only touch state and dispatch. */
function harness(doc: string, cursor: number) {
  ...
  return {
    ...
    act(pos: number, action: TableAction) {
