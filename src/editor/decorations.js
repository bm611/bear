...
class TodoWidget extends WidgetType {
    checked;
    bracketFrom;
    constructor(checked, bracketFrom) {
        super();
        this.checked = checked;
        this.bracketFrom = bracketFrom;
    }
    eq(other) {
 ...
 */
export function headingMarkRanges(text) {
        ...
        tree.iterate({
            ...
            enter: (node) => {
                ...
                if (node.name === 'ListMark') {
                    ...
                    const bullet = BULLET_RE.test(doc.sliceString(node.from, node.to)) && !TODO_RE.test(line.text);
        ...
        while (pos <= to) {
            ...
            if (line.length > 0 && !inCode(view, line.from)) {
                ...
                if (todo) {
                    ...
                    replacements.push(Decoration.replace({
...
export function slateDecorations(options) {
