...
function seedTextForFilter(filter) {
    return filter.kind === 'tag' ? `# \n\n#${filter.tag}` : '# ';
}
function touch(note, text) {
...
export const useStore = create((set, get) => ({
    ...
    updateNoteText: (id, text) => set((state) => {
    ...
    toggleArchive: (id) => set((state) => ({
        notes: state.notes.map((note) => note.id === id ? { ...note, archived: !note.archived, trashedAt: null } : note),
    })),
    trashNote: (id) => set((state) => ({
        notes: state.notes.map((note) => note.id === id ? { ...note, trashedAt: Date.now(), pinned: false } : note),
        selectedId: state.selectedId === id ? nextSelection(state.notes, id, state.filter) : state.selectedId,
    })),
    restoreNote: (id) => set((state) => ({
        notes: state.notes.map((note) => note.id === id ? { ...note, trashedAt: null, archived: false } : note),
    })),
    deleteForever: (id) => set((state) => ({
...
function nextSelection(notes, removedId, filter) {
